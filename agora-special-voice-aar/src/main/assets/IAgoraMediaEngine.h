...
enum AUDIO_EXTERNAL_SOURCE_ID {
  /** External audio playout source
   */
  AUDIO_EXTERNAL_PLAYOUT_SOURCE = 0,
  /** Place external audio recording source before processing
   */
  AUDIO_EXTERNAL_RECORD_SOURCE_PRE_PROCESS = 1,
  /** Place external audio recording source after processing
   */
...
class IMediaEngine {
 public:
  ...
  /** Pushes the external audio frame to the specific position.

   @param frame Pointer to the audio frame: \ref IAudioFrameObserver::AudioFrame "AudioFrame".
   @param sourceId SourceId of external audio frame: #AUDIO_EXTERNAL_SOURCE_ID.

   @return
   - 0: Success.
   - < 0: Failure.
   */
  virtual int pushAudioFrame(IAudioFrameObserver::AudioFrame* frame, int32_t sourceId) = 0;
  ...
  */
  ...
  /** Pushes the video frame using the \ref ExternalVideoFrame
   "ExternalVideoFrame" and passes the video frame to the Agora SDK.

   @param frame Video frame to be pushed. See \ref ExternalVideoFrame
   "ExternalVideoFrame".

   @note In the `COMMUNICATION` profile, this method does not support video
   frames in the Texture format.
